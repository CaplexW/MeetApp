...
export default function MainPage() {
  ...
  return (
    <div className="row gutter-sm p-3 d-flex justify-content-center align-items-center">
      {/* <h1 className="mx-5 my-3">Найди себе компанию!</h1> */}
      <div className="m-5 align-items-center">
        <Link to="/login"><button className="btn btn-primary col-sm-3 m-1" type="button">Войти</button></Link>
        <Link to="/login/register"><button className="btn btn-primary col-sm-3 m-1" type="button">Зарегестрироваться</button></Link>
        <button className="btn btn-info col-sm-3 m-1" disabled={clicked} onClick={activeGuestMode} type="button">Осмотреться</button>
      </div>
    </div>
  );
}
